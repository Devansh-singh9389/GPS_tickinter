...
class CompilerView(tk.Frame):
    ...
    def _build_ui(self, container):
        ...
        tk.Label(hdr, text="Compile source code for simulation", font=FONT_BODY, fg=C_MUTED, bg=C_BG).pack(
            side=tk.RIGHT)
        ...
        self._path_row(panel, 1, "Source directory", "Source Code", "The folder containing gpssim.c and gpssim.h.",
                       self.src_dir_var, self.ctrl.browse_source)

        FieldLabel(panel, "User Motion Size (s)", "Simulation Duration",
                   "Maximum simulation duration in seconds (default: 3000).").grid(row=2, column=0, sticky="w", pady=5)
        spin = tk.Spinbox(panel, from_=1, to=86400, increment=100, textvariable=self.user_motion_size_var,
                          bg=C_ENTRY_BG, fg=C_TEXT, buttonbackground=C_BORDER, relief=tk.FLAT, width=10, font=FONT_BODY)
        spin.grid(row=2, column=1, sticky="w", pady=5, padx=(8, 0))

        FieldLabel(panel, "Optimization", "Compiler Optimization",
                   "-O0 to -O3 flag. Higher means faster simulation but longer compile time.").grid(row=3, column=0,
                                                                                                    sticky="w", pady=5)
        ttk.Combobox(panel, textvariable=self.optimization_var, values=list(OPTIMIZATION_OPTIONS.keys()),
                     state="readonly", width=20).grid(row=3, column=1, sticky="w", pady=5, padx=(8, 0))

        # --- OFFICIAL COMPILER FEATURES SECTION ---
        feat_box = tk.LabelFrame(panel, text="Official Compiler Features", bg=C_PANEL, fg=C_ACCENT,
        ...
        c1 = tk.Checkbutton(feat_box, text="Large File Support (>2GB I/Q)", variable=self.large_file_var, bg=C_PANEL,
        ...
        attach_advanced_tooltip(c1, "Large File Support (-D_FILE_OFFSET_BITS=64)",
                                "Generating GPS I/Q files creates massive binary data files (often multiple gigabytes). This ensures older or 32-bit systems won't crash.")

        c2 = tk.Checkbutton(feat_box, text="Multi-Threading (Real-time SDR)", variable=self.multithread_var, bg=C_PANEL,
                            fg=C_TEXT, selectcolor=C_ENTRY_BG, activebackground=C_PANEL, activeforeground=C_TEXT,
                            cursor="hand2")
        c2.grid(row=0, column=1, sticky="w", padx=(0, 20), pady=4)
        attach_advanced_tooltip(c2, "Multi-Threading Flag (-lpthread)",
                                "If using a fork designed for real-time streaming, this links the POSIX threads library so it can calculate and stream simultaneously.")

        c3 = tk.Checkbutton(feat_box, text="Native CPU Architecture", variable=self.arch_native_var, bg=C_PANEL,
                            fg=C_TEXT, selectcolor=C_ENTRY_BG, activebackground=C_PANEL, activeforeground=C_TEXT,
                            cursor="hand2")
        c3.grid(row=1, column=0, sticky="w", padx=(0, 20), pady=4)
        attach_advanced_tooltip(c3, "CPU Architecture Optimizations (-march=native)",
                                "Tells the compiler to use every advanced instruction your exact computer CPU supports (like AVX or SSE) for much faster math rendering.")

        c4 = tk.Checkbutton(feat_box, text="Extreme Speed Math", variable=self.fast_math_var, bg=C_PANEL, fg=C_TEXT,
                            selectcolor=C_ENTRY_BG, activebackground=C_PANEL, activeforeground=C_TEXT, cursor="hand2")
        c4.grid(row=1, column=1, sticky="w", padx=(0, 20), pady=4)
        attach_advanced_tooltip(c4, "Fast Math (-ffast-math)",
                                "Breaks strict IEEE floating-point math rules for extreme speeds. Note: can occasionally cause minor math rounding errors.")

        # --- DYNAMIC EXTRA CFLAGS LIST ---
        FieldLabel(panel, "Extra CFLAGS", "Custom Build Flags",
                   "Enable specific C compiler flags to modify how the executable is built.").grid(row=5, column=0,
                                                                                                   sticky="nw", pady=5)
        ...
        for flag_text, start_checked, t_title, t_desc in default_flags:
            self._add_flag_row(default_text=flag_text, is_default=True, start_checked=start_checked,
                               tooltip_title=t_title, tooltip_desc=t_desc)

        self.add_flag_btn = GhostButton(self.flags_main_frame, text="+ Add Custom Flag",
                                        command=lambda: self._add_flag_row(""))
        self.add_flag_btn.pack(anchor="w", pady=(4, 0))

        FieldLabel(panel, "Output executable", "Binary Name", "The name of the resulting compiled program.").grid(row=6,
                                                                                                                  column=0,
                                                                                                                  sticky="w",
                                                                                                                  pady=5)
        StyledEntry(panel, textvariable=self.output_exe_var).grid(row=6, column=1, columnspan=2, sticky="ew", pady=5,
                                                                  padx=(8, 0))

        self._path_row(panel, 7, "Output directory", "Build Destination",
                       "Where the compiled executable will be saved.", self.output_dir_var, self.ctrl.browse_output_dir)

        self._build_header_config(panel, 8)

        tk.Checkbutton(panel, text="Clean before build", variable=self.clean_before_build_var, bg=C_PANEL, fg=C_TEXT,
                       activebackground=C_PANEL, activeforeground=C_ACCENT, selectcolor=C_ENTRY_BG, font=FONT_BODY,
                       cursor="hand2").grid(row=9, column=0, columnspan=3, sticky="w", pady=(6, 2))

        mode_row = tk.Frame(panel, bg=C_PANEL)
        mode_row.grid(row=10, column=0, columnspan=3, sticky="w", pady=4)
        for text, value in [("Auto (prefer Makefile)", "auto"), ("Force make", "force_make"),
                            ("Force gcc", "force_gcc")]:
            tk.Radiobutton(mode_row, text=text, value=value, variable=self.build_mode_var, bg=C_PANEL, fg=C_TEXT,
                           activebackground=C_PANEL, activeforeground=C_ACCENT, selectcolor=C_ENTRY_BG, font=FONT_BODY,
        ...
        tk.Label(action, textvariable=self.exe_status_var, fg=C_MUTED, bg=C_PANEL, font=("Courier New", 9)).pack(
            side=tk.LEFT, padx=(12, 0))

        self.process_progress = ttk.Progressbar(action, mode="indeterminate", style="Accent.Horizontal.TProgressbar",
                                                length=170)
        ...
        self.log = scrolledtext.ScrolledText(log_frame, height=11, bg=C_ENTRY_BG, fg=C_TEXT, font=FONT_MONO,
    ...
    def _build_header_config(self, parent, row):
        box = tk.LabelFrame(parent, text="Advanced Header Configuration (gpssim.h)", bg=C_PANEL, fg=C_ACCENT,
                            font=("Courier New", 9, "bold"), padx=10, pady=8, highlightbackground=C_BORDER)
        box.grid(row=row, column=0, columnspan=3, sticky="ew", pady=(8, 4))
        box.columnconfigure(1, weight=1)
        box.columnconfigure(3, weight=1)

        phase = tk.Checkbutton(box, text="Enable high-precision carrier phase", variable=self.float_carr_phase_var,
        ...
        tk.Spinbox(box, from_=1, to=86400, textvariable=self.user_motion_size_var, width=10, bg=C_ENTRY_BG, fg=C_TEXT,
                   buttonbackground=C_BORDER, relief=tk.FLAT).grid(row=1, column=1, sticky="w", padx=(8, 14))

        FieldLabel(box, "Max satellites").grid(row=1, column=2, sticky="w")
        tk.Spinbox(box, from_=8, to=40, textvariable=self.max_sat_var, width=7, bg=C_ENTRY_BG, fg=C_TEXT,
                   buttonbackground=C_BORDER, relief=tk.FLAT).grid(row=1, column=3, sticky="w", padx=(8, 0))

        FieldLabel(box, "Max channels").grid(row=2, column=0, sticky="w")
        tk.Spinbox(box, from_=4, to=32, textvariable=self.max_chan_var, width=10, bg=C_ENTRY_BG, fg=C_TEXT,
                   buttonbackground=C_BORDER, relief=tk.FLAT).grid(row=2, column=1, sticky="w", padx=(8, 14), pady=4)

        GhostButton(box, text="Restore Header Backup", command=self.ctrl.restore_header).grid(row=2, column=2,
    ...
    def _add_flag_row(self, default_text="", is_default=False, start_checked=True, tooltip_title="", tooltip_desc=""):
        ...
        if not is_default:
            del_btn = tk.Button(row_frame, text="✖", bg=C_PANEL, fg=C_ERROR, activebackground=C_PANEL,
                                activeforeground=C_ERROR, relief=tk.FLAT, cursor="hand2",
                                command=lambda f=row_frame, tv=toggle_var, sv=text_var: self._remove_flag_row(f, tv,
                                                                                                              sv))
