...
class GeneratorView(tk.Frame):
    ...
    def _build_ui(self, container):
        ...
        self._path_row(panel, 1, "Executable Path", "Compiler Output",
                       "The gps-sdr-sim executable from the Compiler Tab.", self.exe_var, self.ctrl.browse_exe,self.ctrl.select_exe_file)
        self._path_row(panel, 2, "Navigation File (-e)", "Required Ephemeris",
                       "Specifies the path to your RINEX navigation file.", self.nav_file_var, self.ctrl.browse_nav,
                       self.ctrl.select_nav_file)

        # --- AUTO-NAMING OUTPUT FILE ROW ---
        FieldLabel(panel, "Output File (-o)", "Smart Output Name",
                   "Auto-generates based on parameters. Click Auto-Name to reset.").grid(row=3, column=0, sticky="w",
                                                                                         pady=5, padx=(0, 8))
        ...
        self.llh_lat_var, self.llh_lon_var, self.llh_hgt_var = tk.StringVar(value="35.681236"), tk.StringVar(
            value="139.767125"), tk.StringVar(value="40")
        ...
        mode_box.bind("<<NotebookTabChanged>>",
                      lambda e: self.motion_mode_var.set(["static", "csv", "nmea"][mode_box.index(mode_box.select())]))

        # --- 3. SIGNAL & TIME PARAMETERS SECTION ---
        sig_box = tk.LabelFrame(panel, text="3. Hardware & Time Adjustments", bg=C_PANEL, fg=C_ACCENT,
        ...
        FieldLabel(sig_box, "Duration (-d)", "Simulation Length",
                   "Length in seconds. Max static is 86,400s (24h), dynamic is 300s (5m).").grid(row=0, column=0,
                                                                                                 sticky="w", pady=4)
        tk.Spinbox(sig_box, from_=1, to=86400, increment=100, textvariable=self.duration_var, bg=C_ENTRY_BG, fg=C_TEXT,
                   buttonbackground=C_BORDER, relief=tk.FLAT, width=12).grid(row=0, column=1, sticky="w", pady=4,
                                                                             padx=(8, 20))

        FieldLabel(sig_box, "Sample Rate (-s)", "Hardware Frequency",
                   "Base sample rate in Hz. HackRF defaults to 2600000. BladeRF/LimeSDR often need 5000000+.").grid(
            row=0, column=2, sticky="w", pady=4)
        StyledEntry(sig_box, textvariable=self.sample_rate_var, width=14).grid(row=0, column=3, sticky="w", pady=4,
                                                                               padx=(8, 20))

        FieldLabel(sig_box, "I/Q Bits (-b)", "Bit Depth", "1, 8, or 16 bits.\nHackRF = 8\nBladeRF/USRP = 16").grid(
            row=0, column=4, sticky="w", pady=4)
        ttk.Combobox(sig_box, textvariable=self.iq_bits_var, values=["1", "8", "16"], state="readonly", width=5).grid(
            row=0, column=5, sticky="w", pady=4, padx=(8, 0))

        FieldLabel(sig_box, "Start Time (-t)", "Override Start",
                   "Format: YYYY/MM/DD,hh:mm:ss\nOverrides the exact day and clock time the simulation begins.").grid(
            row=1, column=0, sticky="w", pady=4)
        StyledEntry(sig_box, textvariable=self.start_time_var, width=18).grid(row=1, column=1, sticky="w", pady=4,
                                                                              padx=(8, 20))

        FieldLabel(sig_box, "TOC/TOE Override (-T)", "Override TOC/TOE",
                   "Format: YYYY/MM/DD,hh:mm:ss\nOverwrites the internal Time of Clock (TOC) and Time of Ephemeris (TOE).").grid(
            row=1, column=2, sticky="w", pady=4)
        StyledEntry(sig_box, textvariable=self.toc_time_var, width=18).grid(row=1, column=3, sticky="w", pady=4,
                                                                            padx=(8, 20))

        FieldLabel(sig_box, "Leap Second (-L)", "Future Leap Second",
                   "Format: wnslf,dn,dtslf\nManually pushes a future leap-second event into the stream.").grid(row=1,
                                                                                                               column=4,
                                                                                                               sticky="w",
                                                                                                               pady=4)
        StyledEntry(sig_box, textvariable=self.leap_sec_var, width=14).grid(row=1, column=5, sticky="w", pady=4,
                                                                            padx=(8, 0))

        # --- 4. ADVANCED PHYSICS SECTION ---
        phys_box = tk.LabelFrame(panel, text="4. Advanced Physics & Debugging", bg=C_PANEL, fg=C_ACCENT,
                                 font=("Courier New", 9, "bold"), padx=10, pady=8, highlightbackground=C_BORDER)
        ...
        c1 = tk.Checkbutton(phys_box, text="Disable Ionospheric Math (-i)", variable=self.disable_iono_var, bg=C_PANEL,
        ...
        attach_advanced_tooltip(c1, "Disable Ionosphere (-i)",
                                "Disables ionospheric delay math. Useful for troubleshooting.")

        c2 = tk.Checkbutton(phys_box, text="Flat Power Levels (-p)", variable=self.disable_pathloss_var, bg=C_PANEL,
                            fg=C_TEXT, selectcolor=C_ENTRY_BG, activebackground=C_PANEL, activeforeground=C_TEXT,
                            cursor="hand2")
        c2.grid(row=0, column=1, sticky="w", padx=(0, 20))
        attach_advanced_tooltip(c2, "Disable Path Loss (-p)", "Keeps transmission power perfectly flat and solid.")

        c3 = tk.Checkbutton(phys_box, text="Verbose Mode (-v)", variable=self.verbose_var, bg=C_PANEL, fg=C_TEXT,
        ...
        attach_advanced_tooltip(c3, "Verbose Mode (-v)",
                                "Prints a real-time breakdown of every single tracking channel.")
        ...
        self.process_progress = ttk.Progressbar(pb_frame, variable=self.progress_var, mode="determinate", maximum=100.0,
        ...
        self.log = scrolledtext.ScrolledText(log_frame, height=9, bg=C_ENTRY_BG, fg=C_TEXT, font=FONT_MONO,
    ...
    def _build_static_tab(self, nb):
        ...
        tk.Radiobutton(t, text="Lat/Lon/Height (-l)", value="llh", variable=self.static_coord_var, bg=C_PANEL,
                       fg=C_TEXT, selectcolor=C_ENTRY_BG).grid(row=0, column=0, sticky="w")
        tk.Radiobutton(t, text="ECEF X/Y/Z (-c)", value="ecef", variable=self.static_coord_var, bg=C_PANEL, fg=C_TEXT,
                       selectcolor=C_ENTRY_BG).grid(row=0, column=1, sticky="w")
    ...
    def _build_csv_tab(self, nb):
        ...
        tk.Radiobutton(t, text="ECEF Format (-u)", value="ecef", variable=self.traj_format_var, bg=C_PANEL, fg=C_TEXT,
                       selectcolor=C_ENTRY_BG).grid(row=0, column=0, sticky="w", pady=(0, 6))
        tk.Radiobutton(t, text="LLH Format (-x)", value="llh", variable=self.traj_format_var, bg=C_PANEL, fg=C_TEXT,
                       selectcolor=C_ENTRY_BG).grid(row=0, column=1, sticky="w", pady=(0, 6))
        self._path_row(t, 1, "CSV file", "User Motion CSV",
                       "A file containing ECEF or Lat/Lon trajectory data over time.", self.csv_file_var,
                       self.ctrl.browse_csv)

    def _build_nmea_tab(self, nb):
        t = StyledFrame(nb, padx=10, pady=8)
        nb.add(t, text="NMEA GGA (-g)")
        self._path_row(t, 0, "GGA file", "NMEA Trajectory", "Standard NMEA GGA formatted log file for dynamic routing.",
                       self.nmea_file_var, self.ctrl.browse_nmea)
